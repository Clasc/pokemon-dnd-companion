...
export default function PokemonCard({ pokemon, isEditing, onHPChange, onXPChange }: PokemonCardProps) {
  ...
  return (
    <div className="glass rounded-2xl p-4 mb-3 transition-all duration-300 hover:transform hover:-translate-y-1">
      <div className="flex items-center gap-3">
        {/* Pokemon Sprite/Icon */}
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-white/20 to-white/5 flex items-center justify-center text-2xl border border-white/10">
          {pokemon.sprite || '❓'}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-semibold text-white text-lg truncate">{pokemon.name}</h3>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-gray-300">XP</span>
              <div className="flex items-center gap-2">
                {isEditing && (
                  <>
                    <button
